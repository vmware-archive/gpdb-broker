...
public class SqlServerBrokerTest {
    ...
    public void testCreateAndDeleteInstance() throws ServiceBrokerException {
        serviceInstance.getParameters().put(SqlServerBroker.TOPIC_NAME_KEY, "myTopic");
        Object o = serviceInstance.getParameters().get(SqlServerBroker.TOPIC_NAME_KEY);
        assertNotNull(o);
        assertEquals("myTopic", o.toString());

        String topicName = "topic" + System.currentTimeMillis();
        serviceInstance.getParameters().put(SqlServerBroker.TOPIC_NAME_KEY, topicName);

        kafkaBroker.createInstance(serviceInstance);
        o = serviceInstance.getParameters().get(SqlServerBroker.TOPIC_NAME_KEY);
        assertNotNull(o);
    ...
    public void testGetCredentials() {
        serviceInstance.getParameters().put(SqlServerBroker.TOPIC_NAME_KEY, "foo");
        ...
        assertEquals("foo", m.get(SqlServerBroker.TOPIC_NAME_KEY));  //this assumes you have a topic with the name foo
    }
